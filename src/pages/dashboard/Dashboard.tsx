...
const Dashboard = () => {
  ...
  const links = [
    ...
    {
      title: 'Net revenue', link: 'profit', tooltip: `Revenue generated by the offer minus aggregator's commission, discounts amount, ads CPC and food cost.`
    },
    { title: 'Accrued discount', link: 'accrued_discounts' },
    { title: 'Ads spend', link: 'ads_spend', disabled: true, },
    { title: 'Return on investment', link: 'roi', tooltip: `# AED generated in Profits for every 1 AED spent on discount. Profits are revenue minus aggregator's commission, order discount, ads CPC and food cost.` },
  ]
    ...
    <div className='wrapper'>
      ...
      <div className='block'>
        <TypographyKit className='dashboard-title'>
          {getPeriod(date.titleDate, date.beforePeriod).charAt(0).toUpperCase() + getPeriod(date.titleDate, date.beforePeriod).slice(1)}
          {' '}results for{' '}
          {isDisplay()}
        </TypographyKit>
        <TypographyKit className='dashboard-subtitle'>
          360° view of your restaurant revenue and profits
        </TypographyKit><div className='dashboard-wrapper'>
          {links.filter((obj: { disabled?: boolean }) => !obj.disabled).map((obj: { title: string, link: string, tooltip?: string, }) => <Widget
            table={table}
            setTable={setTable}
            key={obj.link}
            title={obj.title}
            ...
            loading={metricsafterPeriod.length === 0 || metricsbeforePeriod.length === 0 || loading}
            links={links}
            tooltip={obj.tooltip}
          />)}
        </div>
