...
const AdvertsDetails: FC<{ data: any; setOpened: any }> = ({ data, setOpened }) => {
  ...
  const advertDetails = [
    { name: 'Impressions', ...data.impressions, title: data.impressions.title === null ? '-' : data.impressions.title },
    { name: 'Orders', ...data.orders, title: data.orders.title === null ? '-' : data.orders.title },
    { name: 'Clicks', ...data.clicks, title: data.clicks.title === null ? '-' : `${data.clicks.title} AED` },
    { name: 'Conversion Rate', title: `${parseFloat((data.conversion_rate * 100).toFixed(2))}%`, },
    { name: 'New Customers', ...data.customers, title: data.customers.title === null ? '-' : data.customers.title },
    { name: 'Budget Spent', title: data.spend === null ? '-' : `${data.spend} AED` },
    { name: 'Return on Spent', title: data.return_on_ad_spent === null ? '-' : `${data.return_on_ad_spent} AED` },
    { name: 'Attributed order value', title: data.attributed_order_value === null ? '-' : `${data.attributed_order_value} AED` },
  ]
  const getAdvertsRanking = () => (
    <TooltipKit
      ...
      arrow
    >
      <span className='render-row-tooltip small'>
        0
      </span>
    </TooltipKit>
  )
  const advertVisibility = [
    { title: 'Your Swimlane ranking', content: [
    ...
    {
      title: 'Your Keyword ranking', content: [
    ...
    {
      title: 'Your Filters ranking', content: [
  ...
  return (
    <PaperKit className='competition-paper adverts'>
      ...
      <div className='adverts-details_top'>
        ...
        <div>
          <p>
            Offer Status : <span style={{ whiteSpace: 'nowrap' }} className={`competition-status ${data.status}`}>
      ...
      <div className='adverts-details_top-text'>
        ...
        <p>
          Important metrics for you to monitor and track the performance of your ad campaign
        </p>
      </div>
      <div className='adverts-details_widgets'>
        {advertDetails.map((obj) => <AdvertsWidget key={obj.name} {...obj} />)}
      </div>
      <div className='adverts-details_middle-text'>
        <span>
          Your Advert visibility
        </span>
        <p>
          A detailed breakdown of your campaign performance
        </p>
      </div>
      <div className='advert_visibility'>
        {advertVisibility.map((obj) => <AdvertsWidgetCustom key={obj.title} {...obj}/>)}
      </div>
      <div className='adverts-details_middle-text'>
        <span>
          Your Advert Settings
        </span>
        <p>
      ...
      <div className='advert_settings'>
        {advertSettings.map((obj) => <AdvertsWidgetCustom key={obj.title} {...obj} />)}
      </div>
