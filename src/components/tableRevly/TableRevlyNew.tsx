...
const TableRevlyNew: FC<{
  ...
  className
    ...
    const renderRows = () => {
      if (isLoading) return renderCustomSkelton ? renderCustomSkelton.map((r) => (
    ...
    const getActiveLinkWidth = (index: number) => {
    ...
    return (
      <BoxKit className={`competition-box ${className || ''}`} sx={{ width: '100%' }}>
        <PaperKit className='table-paper' sx={{ width: '100%', mb: 2 }}>
          {links ? <div className='table-paper-top'>
            <div style={{ display: 'flex' }}>
              <BoxKit className='table-links'>
                {links.map((name: string, index: number) => (
                  <TypographyKit
                    ...
                    style={{ '--length': `${getActiveLinkWidth(index)}px` }}
              ...
              <BoxKit className='table-arrow-links'>
                <img tabIndex={-1} role='presentation' className={links.findIndex((n) => link === n) > 0 ? 'active' : ''} onClick={() => setLink(links[links.findIndex((n) => link === n) - 1])} src={Arrow} alt='left-arrow' />
                <img tabIndex={-1} role='presentation' className={links.findIndex((n) => link === n) < links.length - 1 ? 'active' : ''} onClick={() => setLink(links[links.findIndex((n) => link === n) + 1])} src={Arrow} alt='right-arrow' />
              </BoxKit>
            </div>
            {setOpenedFilter ? <BoxKit onClick={() => setOpenedFilter(true)} className='table-filter'>
              <span />
              <p>Filters</p>
            </BoxKit> : ''}
          </div> : ''}
