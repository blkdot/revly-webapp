...
const Calendar: FC<CalendarProps> = ({dateRange, events}) => {
  ...
  const eventsByDate = useMemo(() => {
    ...
    events.forEach(event => {
      const dateStart = dayjs(event.start_date).isBefore(datesOfWeek[0]) ? dayjs(datesOfWeek[0]) : dayjs(event.start_date);
      const dateEnd = dayjs(event.end_date).isAfter(datesOfWeek[6]) ? dayjs(datesOfWeek[6]) : dayjs(event.end_date);
      for (let iDate = dateStart; !iDate.isAfter(dateEnd); iDate = iDate.add(1, 'day')) {
        if (!eventsByDateObj[iDate.unix()]) {
          eventsByDateObj[iDate.unix()] = [];
        }

        eventsByDateObj[iDate.unix()].push(event);
      }
    });

    console.log(eventsByDateObj, 'eventsByDateObj');

    return eventsByDateObj;
  }, [events, datesOfWeek]);

  const hasPrevWeek = useMemo(() => dayjs(dateRange.startDate).isBefore(startDate), [dateRange, startDate]);

  const hasNextWeek = useMemo(() => dayjs(dateRange.endDate).isAfter(dayjs(startDate).endOf('week')), [dateRange, startDate]);
  ...
  const renderHeaderDates = () => (
    <BoxKit sx={{
      ...
      <BoxKit width='64px'/>
      <BoxKit width='24px' display='flex' alignItems='center'>
        <ButtonKit
          sx={{
            width: '24px',
            minWidth: 0,
    ...
    <BoxKit sx={{
      ...
      {datesOfWeek.map((date, index) => (
        <BoxKit
          ...
          position='relative'
        >
          {eventsByDate[date.unix()]?.map(event => {
            const startHourSplit = event.start_hour.split(':');
            const startHours = startHourSplit[0];
            const  startMinutes = startHourSplit[1];
            const endHourSplit = event.end_hour.split(':');
            let endHours = endHourSplit[0];
            let endMinutes = endHourSplit[1];
            ...
            const top = startTime.hours() * TIME_SLOT_HEIGHT + startTime.minutes() / 60 * TIME_SLOT_HEIGHT;
            const bottom = (1440 - endTime.asMinutes()) / 60 * TIME_SLOT_HEIGHT;
    ...
    >
          ...
          <div style={{
            ...
            <svg width="10" height="19" viewBox="0 0 10 19" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M8.17718 14.6453C7.52384 14.6453 7.10809 14.532 6.81112 14.1922C6.57354 13.909 6.45475 13.456 6.45475 12.7764V8.6987H9.36505L9.00869 5.64046H6.51415V2.86537V0.600006C6.51415 0.600006 5.38566 0.656647 5.0293 0.713281C3.1287 0.82655 1.88142 2.07249 1.88142 3.43172V5.81036L0.0996094 5.92363V8.75534H1.88142V13.456C1.88142 14.8718 2.29718 16.0045 3.0693 16.854C3.84142 17.7035 5.02929 18.1 6.57353 18.1C7.34565 18.1 7.9396 18.0433 8.47415 17.9867C9.30566 17.8168 9.89959 17.1372 9.89959 16.2877V14.2488C9.30566 14.4754 8.71172 14.6453 8.17718 14.6453Z"
                    fill="white"/>
            </svg>
          </div>
          <div style={{
            ...
            <svg width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M12.4894 9.22134L13.372 0.829086L18.1903 1.33552L16.8881 13.7213L14.1389 19.8708L2.11484 17.3097L0.899414 11.6667L7.72897 10.2197L6.23862 3.14419L10.9846 2.1458L12.4894 9.22134Z"
                fill="white"/>
              <path
                d="M11.9251 13.5187C12.4025 13.7502 12.8656 13.7068 13.0537 13.3885C13.2273 13.0847 13.3141 12.7663 13.1405 12.4769C12.9669 12.1875 12.6775 12.0284 12.3157 12.0429C12.1132 12.0429 11.9251 12.231 11.8093 12.3757C11.7225 12.4769 11.6646 12.6072 11.6501 12.7374C11.6067 12.9834 11.6067 13.3596 11.9251 13.5187Z"
                fill="#35B8B2"/>
              <path
                d="M9.01978 12.7398C9.49727 12.9713 9.96029 12.9279 10.1484 12.6095C10.322 12.3057 10.4088 11.9874 10.2352 11.698C10.0616 11.4086 9.77219 11.2494 9.41046 11.2639C9.20789 11.2639 9.01978 11.452 8.90403 11.5967C8.81721 11.698 8.75933 11.8282 8.74487 11.9584C8.70146 12.2044 8.70146 12.5806 9.01978 12.7398Z"
                fill="#35B8B2"/>
            </svg>
          </div>
          <div style={{
      ...
      <BoxKit sx={{marginLeft: '8px', marginTop: '7px'}}>
        <BoxKit sx={{display: 'flex'}}>
          ...
          <TypographyKit sx={{fontFamily: 'Public Sans', fontSize: '12px', lineHeight: '14px', color: '#1E1E1E'}}>10h00
            - 14h00 (4h)</TypographyKit>
        </BoxKit>
        <BoxKit sx={{marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <BoxKit sx={{display: 'flex', alignItems: 'center'}}>
            ...
            <TypographyKit sx={{fontFamily: 'Public Sans', fontSize: '12px', lineHeight: '14px', color: '#1E1E1E'}}>Menu
              discount</TypographyKit>
          </BoxKit>
          <TypographyKit sx={{
            padding: '3px 7px',
            fontFamily: 'Public Sans',
        ...
        <BoxKit sx={{marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <BoxKit sx={{display: 'flex', alignItems: 'center'}}>
            ...
            <TypographyKit sx={{fontFamily: 'Public Sans', fontSize: '12px', lineHeight: '14px', color: '#1E1E1E'}}>Free
              Item</TypographyKit>
          </BoxKit>
          <TypographyKit sx={{
            padding: '3px 7px',
        ...
        <BoxKit sx={{marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <BoxKit sx={{display: 'flex', alignItems: 'center'}}>
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"
                 style={{marginRight: '6px'}}>
              <path d="M3.5 9.99999C3.5 9.17157 4.61929 8.49999 6 8.49999C7.38071 8.49999 8.5 9.17157 8.5 9.99999"
                    stroke="#14181F" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M9 7.12474C9.88295 7.35621 10.5 7.88483 10.5 8.49993" stroke="#14181F" stroke-width="1.16667"
                    stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M3 7.12474C2.11705 7.35621 1.5 7.88483 1.5 8.49993" stroke="#14181F" stroke-width="1.16667"
                    stroke-linecap="round" stroke-linejoin="round"/>
              <path
                d="M6 6.99999C6.82843 6.99999 7.5 6.32841 7.5 5.49998C7.5 4.67156 6.82843 3.99998 6 3.99998C5.17157 3.99998 4.5 4.67156 4.5 5.49998C4.5 6.32841 5.17157 6.99999 6 6.99999Z"
                stroke="#14181F" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
              <path
                d="M9 5.11803C9.30687 4.84337 9.5 4.44423 9.5 3.99998C9.5 3.17155 8.82843 2.49998 8 2.49998C7.61582 2.49998 7.26538 2.6444 7 2.88192"
                stroke="#14181F" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
              <path
                d="M3 5.11803C2.69313 4.84337 2.5 4.44423 2.5 3.99998C2.5 3.17155 3.17157 2.49998 4 2.49998C4.38418 2.49998 4.73462 2.6444 5 2.88192"
                stroke="#14181F" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <TypographyKit sx={{fontFamily: 'Public Sans', fontSize: '12px', lineHeight: '14px', color: '#1E1E1E'}}>All
              customers</TypographyKit>
          </BoxKit>
        </BoxKit>
      </BoxKit>
      <ButtonKit sx={{
  ...
  useEffect(() => {
    setStartDate(dayjs(dateRange.startDate));
  }, [dateRange]);
